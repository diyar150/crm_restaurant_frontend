...
// Use Sell PDF components
import SellInvoicePDF from '../../components/reports/sell/sellInvoicePDF';
import SellInvoiceDetailsPDF from '../../components/reports/sell/sellInvoiceDetailsPDF';
import { useCompanyInfo } from '../../hooks/useCompanyInfo';

const rowsPerPage = 20;

function SellList({ isDrawerOpen }) {
  ...
  const [openInvoiceInfoPDF, setOpenInvoiceInfoPDF] = useState(false);
  ...
  // fetch agents
  useEffect(() => {
    axiosInstance.get('/user/agent').then(res => setAgents(res.data || []));
  }, []);
...
const fetchInvoices = async (
  ...
  try {
    ...
    const response = await axiosInstance.get('/sell-invoice/filter', { params });
  ...
  // CRUD Handlers
  const handleAddInvoice = () => navigate('/sell-invoice/create');
  const handleEditInvoice = (id) => navigate(`/sell-invoice/edit/${id}`);
  ...
  const handleDeleteConfirm = async () => {
    try {
      await axiosInstance.delete(`/sell-invoice/delete/${selectedInvoiceId}`);
  ...
  const handleShowInvoice = async (id) => {
    ...
    try {
      const [invoiceRes, itemsRes] = await Promise.all([
        ...
        axiosInstance.get(`/sell-item/index?invoice_id=${id}`)
      ]);
  ...
  const fetchAllInvoicesForReport = async () => {
    try {
      ...
      const res = await axiosInstance.get('/sell-invoice/filter', { params });
    ...
    >
      ...
      <Box sx={{ mb: 2 }}>
        ...
        <TextField
          ...
          fullWidth
          label="مەندووب"
          value={filterAgent?.id || ''}
          onChange={e => { setFilterAgent(agents.find(agent => agent.id === e.target.value)); setCurrentPage(1); }}
        >
          <MenuItem value="">هەموو</MenuItem>
          {agents.map((agent) => (
      ...
      <TextField
        ...
        onChange={e => { setFilterDriverId(e.target.value); setCurrentPage(1); }}
      >
        <MenuItem value="">هەموو</MenuItem>
        {employees.map((emp) => (
      ...
      >
        <MenuItem value="">هەموو</MenuItem>
        <MenuItem value="cash">نەقد</MenuItem>
        <MenuItem value="loan">قەرز</MenuItem>
        <MenuItem value="direct">ڕاستەوخۆ</MenuItem>
      ...
      {isLoading ? (
       ...
       <TableHead>
          <TableRow>
            <TableCell onClick={() => handleSort('id')} sx={{ cursor: 'pointer' }}>ID {getSortIcon('id')}</TableCell>
            <TableCell>کڕیار</TableCell>
            <TableCell onClick={() => handleSort('type')} sx={{ cursor: 'pointer' }}>ج.مامەڵە {getSortIcon('type')}</TableCell>
            <TableCell onClick={() => handleSort('currency_id')} sx={{ cursor: 'pointer' }}>دراو {getSortIcon('currency_id')}</TableCell>
            <TableCell onClick={() => handleSort('total_amount')} sx={{ cursor: 'pointer' }}>کۆی گشتی {getSortIcon('total_amount')}</TableCell>
            <TableCell>مەندووب</TableCell>
            <TableCell>شۆفێر</TableCell>
            <TableCell>کارمەند</TableCell>
            <TableCell onClick={() => handleSort('branch_id')} sx={{ cursor: 'pointer' }}>لق {getSortIcon('branch_id')}</TableCell>
            <TableCell onClick={() => handleSort('warehouse_id')} sx={{ cursor: 'pointer' }}>کۆگا {getSortIcon('warehouse_id')}</TableCell>


            <TableCell>بەروار</TableCell>
            <TableCell>جۆری پارەدان</TableCell>
           ...
           <TableBody>
                {invoices.map((invoice) => (
                  <TableRow key={invoice.id} hover>
                    ...
                    <TableCell>
                    {invoice.type === 'ڕاستەوخۆ'
                      ? invoice.direct_customer_name
                        ? `${invoice.direct_customer_name}${invoice.direct_customer_phone ? ` (${invoice.direct_customer_phone})` : ''}`
                        : '-'
                      : invoice.customer_name || (invoice.customer_id === 0 ? '-' : invoice.customer_id)
                    }
                  </TableCell>
                    <TableCell>{invoice.type}</TableCell>
                    <TableCell>{currencies.find(cur => cur.id === invoice.currency_id)?.name || invoice.currency_id || '-'}</TableCell>
                    <TableCell>{Number(invoice.total_amount || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</TableCell>
                    <TableCell>{invoice.agent_name}</TableCell>
                    <TableCell>{invoice.driver_name}</TableCell>
                    <TableCell>{invoice.employee_name}</TableCell> 
                    <TableCell>{branches.find(b => b.id === invoice.branch_id)?.name || invoice.branch_id || '-'}</TableCell>
                    <TableCell>{warehouses.find(w => w.id === invoice.warehouse_id)?.name || invoice.warehouse_id || '-'}</TableCell>
      ...
      <DialogPdf
        ...
        fileName={`sell_invoice_${showInvoice?.invoice_number || showInvoice?.id || ''}.pdf`}
      />

      <DialogPdf
        open={openInvoiceInfoPDF}
        onClose={() => setOpenInvoiceInfoPDF(false)}
        document={
          reportInvoices.length > 0 ? (
            <SellInvoiceDetailsPDF
        ...
        fileName="sell_invoices_report.pdf"
      />
