...
function BuyReturnCreate() {
  ...
  useEffect(() => {
    ...
    axiosInstance.get(`/buy-return-invoice/show/${id}`)
      .then(async res => {
        ...
        const itemsRes = await axiosInstance.get(`/buy-return-item/index?buy_return_invoice_id=${id}`);
        setForm(f => ({
          ...f,
  ...
  const handleSubmit = async e => {
    e.preventDefault();
    await handleStoreOrUpdate('submit');
  };

  // --- Store or update logic ---
  const handleStoreOrUpdate = async (mode = 'submit') => {
    const errors = {};
    ...
    try {
      ...
      let buy_return_invoice_id = id;
      let oldItems = [];
      if (id) {
        await axiosInstance.put(`/buy-return-invoice/update/${id}`, invoicePayload);
        const oldItemsRes = await axiosInstance.get(`/buy-return-item/index?buy_return_invoice_id=${id}`);
        ...
        const invoiceRes = await axiosInstance.post('/buy-return-invoice/store', invoicePayload);
        buy_return_invoice_id = invoiceRes.data.id;
        setIsEditing(false);
      }
      ...
      for (const item of form.items) {
        ...
        if (oldItem && oldItem.id) {
          if (
            ...
            await axiosInstance.put(`/buy-return-item/update/${oldItem.id}`, {
              buy_return_invoice_id,
              item_id: item.item_id?.id || item.item_id,
              item_unit_id: item.item_unit_id,
              quantity: item.quantity,
              unit_price: item.unit_price,
          ...
          await axiosInstance.post('/buy-return-item/store', {
            buy_return_invoice_id,
            item_id: item.item_id?.id || item.item_id,
            item_unit_id: item.item_unit_id,
            quantity: item.quantity,
      ...
      for (const key in oldMapByKey) {
        if (!newMapByKey[key]) {
          await axiosInstance.delete(`/buy-return-item/delete/${oldMapByKey[key].id}`);
        }
      }
      ...
      if (mode === 'submit') {
        ...
        if (!id) {
          ...
          navigate('/buy-return-invoice');
        }
      }
      return buy_return_invoice_id;
    } catch (err) {
    ...
    <Box>
      <Card sx={{ p: 3, mb: 3 }}>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
          <Typography variant="h5" gutterBottom>بەشی گەڕانەوەی کاڵا</Typography>
          <IconButton onClick={() => navigate('/buy-return-invoice')}>
            <CloseIcon />
          </IconButton>
        </Box>
              ...
              {form.items.map((item, idx) => {
                ...
                return (
                  <TableRow key={idx}>
                   ...
                   <TableCell>
                      {Number(isEditingRow ? editingItem.total : item.total || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </TableCell>
                    <TableCell>
                      {isEditingRow ? (
                        <>
...
export default BuyReturnCreate;