...
import BuyInvoicePDF from '../../components/reports/buy/buyInvoicePDF';
import BuyInvoiceDetailsPDF from '../../components/reports/buy/buyInvoiceDetailsPDF';
import { useCompanyInfo } from '../../hooks/useCompanyInfo';

const rowsPerPage = 20;

function BuyList({ isDrawerOpen }) {
...
const [reportInvoices, setReportInvoices] = useState([]);
  ...
  const fetchInvoices = async (
    ...
    try {
      ...
      const response = await axiosInstance.get('/buy-invoice/filter', { params });
  ...
  // CRUD Handlers
  const handleAddInvoice = () => navigate('/buy-invoice/create');
  const handleEditInvoice = (id) => navigate(`/buy-invoice/edit/${id}`);
  ...
  const handleDeleteConfirm = async () => {
    try {
      await axiosInstance.delete(`/buy-invoice/delete/${selectedInvoiceId}`);
  ...
  const handleShowInvoice = async (id) => {
    ...
    try {
      const [invoiceRes, itemsRes] = await Promise.all([
        ...
        axiosInstance.get(`/buy-item/index?buy_invoice_id=${id}`)
      ]);
...
const handleClearAllFilters = () => {
  ...
  const fetchAllInvoicesForReport = async () => {
    try {
      ...
      const res = await axiosInstance.get('/buy-invoice/filter', { params });
...
>
  ...
  >
    ...
    >
      ...
      <DialogPdf
        ...
        fileName={`buy_invoice_${showInvoice?.invoice_number || showInvoice?.id || ''}.pdf`}
      />
  ...
  <DialogPdf
    ...
    fileName="buy_invoices_report.pdf"
  />
